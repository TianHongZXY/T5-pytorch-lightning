...
class ModelArguments:
    ...
    """
    model_t5_type:Optional[str] = field(
        default='t5',
        metadata={"help":"load T5 or MT5 model type"}
    )
...
def main():
    ...
    if model_args.config_name:
        if model_args.model_t5_type == 't5':
            config = T5Config.from_pretrained(
                model_args.config_name, cache_dir=model_args.cache_dir, vocab_size=len(tokenizer)
            )
        elif model_args.model_t5_type == 'mt5':
            config = MT5Config.from_pretrained(
                model_args.config_name, cache_dir=model_args.cache_dir, vocab_size=len(tokenizer)
            )
        else:
            logger.ValueError("Please instantiate model type of t5 or mt5.")
    elif model_args.model_name_or_path:
        if model_args.model_t5_type == 't5':
    ...
    if model_args.model_name_or_path:
        ...
        if model_args.model_t5_type == 't5':
            ...
            model = FlaxMT5ForConditionalGeneration(config, seed=training_args.seed, dtype=getattr(jnp, model_args.dtype))
